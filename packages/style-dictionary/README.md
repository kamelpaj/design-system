...
(Template cred: https://github.com/amzn/style-dictionary/tree/main/examples/advanced/multi-brand-multi-platform)
...
First of all, set up the required dependencies running the command `yarn install` in your local CLI environment.

At this point, if you want to build the tokens you can run `yarn build`. This command will generate the files in the `build` folder. Unlike other examples, the files are organised not only by "platform", but also organised in "brand" sub-folders.
